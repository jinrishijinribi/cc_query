...
def sync_data():
    print(get_config('sync_tag')[0]['value'])
    sync_tag = get_config('sync_tag')[0]['value']
    if not sync_tag == 'True':
        set_config('sync_tag', True)
        sync_db_by_contract()
        sync_db_by_wallet()
        set_config('sync_tag', False)
    return "success"
...
# 合约地址新建
@app.route("/api/address/create", methods=['POST'])
...
# 充值释放bnb
@app.route("/api/manual/charge", methods=['POST'])
def manual_charge():
    data = request.json
    print(data)
    bnb_in = data['bnb_in']
...
# 充值/释放删除
@app.route("/api/manual/charge/delete", methods=['POST'])
...
# 充值/释放列表
@app.route("/api/manual/charge/list/<tp>", methods=['GET'])
...
# 首页统计
@app.route("/api/index/stat", methods=['GET'])
def index_stat():
...
# 统计/用户cc币持仓成本
@app.route("/api/stat/address/cc", methods=['POST'])
def address_cc():
    data = request.json
    page = data["page"]
    size = data["size"]
...
# 交易详情 trade_detail
@app.route("/api/stat/trade/log", methods=['POST'])
def trade_log():
    data = request.json
